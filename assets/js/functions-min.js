    ...
    function s() {
      ...
      for (let i = 0; i < $slider.length; i++) {
        ...
        const initializeSlider = () => {
          $slides.removeClass(
            "slider--item-left slider--item-center slider--item-right"
          );
          if (totalSlides > 0) $slides.eq(0).addClass("slider--item-left");
          if (totalSlides > 1) $slides.eq(1).addClass("slider--item-center");
          if (totalSlides > 2) $slides.eq(2).addClass("slider--item-right");
        };

        const updateClasses = (direction) => {
          console.log(totalSlides);
          // Get current indices
          const currentLeft = $slides.index($($slider[i]).find(".slider--item-left"));
          const currentCenter = $slides.index(
            $($slider[i]).find(".slider--item-center")
          );
          const currentRight = $slides.index(
            $($slider[i]).find(".slider--item-right")
          );
          ...
          const newLeft =
            direction === "next"
              ? (currentLeft + 1) % totalSlides
              : (currentLeft - 1 + totalSlides) % totalSlides;

          const newCenter =
            direction === "next"
              ? (currentCenter + 1) % totalSlides
              : (currentCenter - 1 + totalSlides) % totalSlides;

          const newRight =
            direction === "next"
              ? (currentRight + 1) % totalSlides
              : (currentRight - 1 + totalSlides) % totalSlides;
          ...
          $slides.eq(newLeft).addClass("slider--item-left");
          $slides.eq(newCenter).addClass("slider--item-center");
          $slides.eq(newRight).addClass("slider--item-right");
        };

        const handleButtonClick = (direction) => {
          // Prevent multiple animations causing issues
        ...
        $($sl.parent().children()[1]).click(() => handleButtonClick("prev"));
        $($sl.parent().children()[2]).click(() => handleButtonClick("next"));
